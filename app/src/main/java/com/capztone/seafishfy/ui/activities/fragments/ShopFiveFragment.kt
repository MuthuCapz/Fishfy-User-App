...
class ShopFiveFragment : Fragment() {
    ...
    private lateinit var binding: FragmentShopFiveBinding
    private val viewModel: ShopFiveViewModel by viewModels()

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {
        binding = FragmentShopFiveBinding.inflate(inflater, container, false)

        observeViewModel()

        binding.menuItemTextView.setOnClickListener {
            val bottomSheetDialog = MenuBottomSheetFragment4()
    ...
    private fun setMenu3ItemAdapter(subsetMenuItems: List<MenuItem>) {
        val adapter = MenuAdapter(subsetMenuItems, requireContext())
        binding.popularRecyclerView3.layoutManager =
            LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
    ...
    private fun setDiscountItemAdapter(discountItems: List<DiscountItem>) {
        ...
        binding.discountRecyclerView.layoutManager =
            LinearLayoutManager(requireContext(), LinearLayoutManager.HORIZONTAL, false)
        ...
        if (hour in 7 until 17) {
            ...
            binding.discountRecyclerView.setOnClickListener {
                ToastHelper.showCustomToast(
                    requireContext(),
                    "Discount items opens only after 5 PM"
                )
            }
        } else {
